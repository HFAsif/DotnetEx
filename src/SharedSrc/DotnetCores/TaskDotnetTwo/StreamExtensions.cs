...
namespace System.IO
{
    ...
    public static class StreamExtensions {
        ...
        /// </exception>
        public static Task CopyToAsync( Stream stream, Stream destination) {
#if NET45PLUS
            if (stream == null)
        ...
        /// <para>If <paramref name="destination"/> does not support writing.</para>
        /// </exception>
        public static Task CopyToAsync( Stream stream, Stream destination, int bufferSize) {
#if NET45PLUS
            if (stream == null)
            throw new ArgumentNullException("stream");
        ...
        /// <para>If <paramref name="destination"/> does not support writing.</para>
        /// </exception>
        public static Task CopyToAsync( Stream stream, Stream destination, int bufferSize, CancellationToken cancellationToken) {
            if(stream == null)
                throw new ArgumentNullException("stream");
            if(destination == null)
                throw new ArgumentNullException("destination");
        ...
        class AsyncState_Stream {
            ...
            /// 对buf的读取数值
            /// </summary>
            public Action<AsyncState_Stream, int> onAsyncRead;
        }
        static void onReadStreamAsync(IAsyncResult asyncResult) {
            AsyncState_Stream ass = asyncResult.AsyncState as AsyncState_Stream;
            Stream fs = ass.stream;
            byte[] data = ass.buf;
            int bufferSize = ass.bufferSize;
            int bytesRead = fs.EndRead(asyncResult);
            //输出到工作台
            //Console.Write(Encoding.ASCII.GetString(data, 0, bytesRead));
            //不断循环,直到读取完毕
            if(bytesRead > 0) {
                if(ass.onAsyncRead != null) {
                    ass.onAsyncRead(ass, bytesRead);
                }
                fs.BeginRead(data, 0, bufferSize, onReadStreamAsync, ass);
            }
            else {
                //    fs.Close(); //当全部读取完毕,显式释放资源
                if(ass.onAsyncRead != null) {
                    ass.onAsyncRead(ass, 0);
                }
            }
        }
        private static Task CopyToAsync(Stream stream, Stream destination, byte[] buffer, CancellationToken cancellationToken) {
            ...
            int bytesRead;
            while((bytesRead = TaskExtensions.Await(ReadAsync(stream, buffer, 0, buffer.Length, cancellationToken)) ) != 0) {
                TaskExtensions.Await(WriteAsync(destination, buffer, 0, bytesRead, cancellationToken) );
            }
            return Task.FromResult(true);
            //
...
#pragma warning restore CS0162 // Unreachable code detected
                              //异步读
            AsyncState_Stream ass = new AsyncState_Stream() {
                buf = buffer,
                bufferSize = buffer.Length,
                stream = stream
            };
            ass.onAsyncRead = (ret, br) => {
                bytesRead = br;
                if(br == 0) {
                    ts.SetResult(true);
                }
                else {
                    destination.Write(ret.buf, 0, bytesRead);
                }

            };
            AsyncCallback callback = new AsyncCallback(onReadStreamAsync);
            IAsyncResult async = stream.BeginRead(buffer, 0, buffer.Length, callback, ass);
            return ts.Task;
        }
#else
        ...
        public static Task FlushAsync( Stream stream) {
#if NET45PLUS
            if (stream == null)
            throw new ArgumentNullException("stream");
        ...
        public static Task WriteAsync( Stream stream, byte[] buffer, int offset, int count) {
#if NET45PLUS
            if (stream == null)
            throw new ArgumentNullException("stream");
